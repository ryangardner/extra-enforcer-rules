...
public class RequireEncoding implements EnforcerRule {
    ...
    /**
     * Comma (or pipe) separated list of globs to include.
     */
    private String includes = "";

    /**
     * Comma (or pipe) separated list of globs to exclude.
     */
    ...
    /**
     * Should the rule fail after the first error or should the errors be
     * aggregated.
     */
    ...
    public void execute(EnforcerRuleHelper helper) throws EnforcerRuleException {
        try {
            if (StringUtils.isBlank(encoding)) {
                setEncoding((String) helper.evaluate("${project.build.sourceEncoding}"));
            }
            ...
            for (String file : ds.getIncludedFiles()) {
                ...
                if (fileEncoding != null && !fileEncoding.equals(encoding)) {
                    ...
                    filesInMsg.append("\n");
                    if (failFast) { throw new EnforcerRuleException(filesInMsg.toString()); }
                }
            }
            if (filesInMsg.length() > 0) { throw new EnforcerRuleException("Files not encoded in " + encoding + ":\n" + filesInMsg); }
    ...
    protected String getEncoding(File file, Log log) throws IOException {
